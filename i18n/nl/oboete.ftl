cancel = Annuleren
save = Opslaan
empty-page = Hier is nog niets
empty-page-noset = Een studieset aanmaken
backup = Backup
import = Backup importeren
repository = Repository
support = Ondersteuning

<#-- Appearance -->
appearance = Weergave
theme = Thema
match-desktop = Systeemstandaard
dark = Donker
light = Licht

<#-- Settings -->
settings = Instellingen

<#-- Application MenuBar -->
file = Bestand
new-studyset = Nieuw studieset
edit = Bewerken
rename-studyset = Studieset hernoemen
delete-studyset = Studieset verwijderen
view = Beeld
about = Over

<#-- Page Headers -->
folders = Mappen
flashcards = Flitskaartjes
study = Studeren
new = Nieuwe
options = Opties

<#-- Delete StudySet Dialog -->
confirm-delete = Studieset verwijderen?
ok = Verwijderen

<#-- New Folder Dialog -->
create-folder = Map aanmaken
rename-folder = Map hernoemen
folder-name = Mapnaam

<#-- Folder Details Context Page -->
folder-details = Details over de map

<#-- Flashcards Page -->
ok-status = Oké
good-status = Goed
bad-status = Slecht

<#-- Flashcard Create/Edit Context Page -->
flashcard-front-title = Voorkant flitskaartje
flashcard-back-title = Achterkant flitskaartje
flashcard-front-placeholder = Voorkant: vul hier je begrip in
flashcard-back-placeholder = Achterkant: hier vul je de betekenis in
create = Aanmaken
reset-flashcard-title = De status (goed/slecht) resetten
reset-flashcard-button = Resetten
